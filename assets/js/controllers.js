...
angular.module('myApp.controllers', ['ui.bootstrap'])
	...
	.controller('LayoutController', ['$scope', '$sails', '$location', '$rootScope',  function($scope, $sails, $location, $rootScope ) {
		...
		$scope.search = function(){
			$location.path('/search?key='+$scope.search_key);
		};
	...
	.controller('searchCtrl', ['$scope', '$sails', '$location', function($scope, $sails, $location ) {	
		var key = $location.absUrl().substring($location.absUrl().indexOf('?') + 4);
		$sails.get('/global/search', {key:key}).success(function (result) {
			if(result.hits != null){
				alert('-----hits----'+ JSON.stringify(result.hits));
			}
		}).error(function (err) {
			alert('/global/search, we got a problem!');
		});	
	}])
